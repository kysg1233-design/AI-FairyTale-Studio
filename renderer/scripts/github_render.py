...
SECRET = os.environ.get("WORKER_JOB_KEY","")
...
def main():
    """GitHub-only: all inputs come from a private job branch, output via Actions Artifact."""
    root=Path(os.environ.get("GITHUB_WORKSPACE",".")).resolve()
    job_dir=root/"jobs"/JOB
    manifest=json.loads((job_dir/"manifest.json").read_text(encoding="utf-8"))
    videos=manifest["videos"]
    count=int(manifest["cuts"])
    if count not in (9,12,15,18,20) or len(videos)!=count:
        raise RuntimeError("Invalid job manifest or cut count")
    with tempfile.TemporaryDirectory(prefix="fairytale-") as tmp:
        wd=Path(tmp)
        completed=[]
        for i,v in enumerate(videos):
            original=(root/v["path"]).resolve()
            if not original.is_relative_to(job_dir) or not original.is_file():
                raise RuntimeError("Invalid private cut path")
            print(f"Rendering cut {i+1}/{count}",flush=True)
            voice=wd/f"voice_{i:02d}.wav"
            speech(v["narration"],manifest["voice"],voice)
            ass=wd/f"subtitle_{i:02d}.ass"
            captions(ass,v["narration"],duration(voice))
            out=wd/f"cut_{i:02d}.mp4"
            render_cut(i,original,voice,ass,out,manifest["backgroundVolume"],
                       manifest["narrationVolume"],manifest.get("quality","balanced"))
            completed.append(out)
        playlist=wd/"list.txt"
        playlist.write_text("".join("file '"+str(path)+"'\\n" for path in completed),encoding="utf-8")
        finished=root/"fairytale-final.mp4"
        run(["ffmpeg","-y","-hide_banner","-loglevel","error","-f","concat","-safe","0",
             "-i",playlist,"-c","copy","-movflags","+faststart",finished])
        final=probe(finished)
        types={x["codec_type"] for x in final["streams"]}
        if finished.stat().st_size<10000 or not {"video","audio"}<=types:
            raise RuntimeError("Final MP4 validation failed")
        if duration(finished)<sum(duration(p) for p in completed)-0.4:
            raise RuntimeError("Final MP4 duration is unexpectedly short")
        print("Validated MP4:",finished.stat().st_size,"bytes",count,"cuts",flush=True)
